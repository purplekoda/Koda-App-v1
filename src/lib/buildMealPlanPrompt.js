...
export async function buildMealPlanPrompt(userId) {
  ...
  try {
    if (isMockMode()) {
      const { getMockOnboardingProfile, getMockGroceryPreferences } = await import('@/lib/dal/mock-store')
      ...
      if (style === 'deal_hunter') {
        shoppingLines.push('This user prefers to shop at multiple stores to find the best prices. Split grocery lists across stores to optimize value and flag when a specific item would be significantly cheaper at a different store the user already shops at.')
      } else if (style === 'convenience') {
        shoppingLines.push('This user prefers convenience over savings. Consolidate grocery lists to their primary store or fewest stores possible. Do not split lists across many stores even if savings exist.')
      } else if (style === 'pickup_only') {
        shoppingLines.push('This user only shops at stores with curbside pickup. Only include stores that offer pickup in grocery list suggestions. Always default the How field on the grocery list screen to Pickup.')
      } else if (style === 'delivery_preferred') {
        shoppingLines.push(`This user prefers grocery delivery${deliverySvc ? ` via ${deliverySvc}` : ''}. Prioritize stores that offer delivery. Default the How field on the grocery list screen to Delivery and pre-select their preferred delivery service.`)
      } else if (style === 'flexible') {
        shoppingLines.push('This user has no strong shopping preference. Let them decide each week how they want to shop.')
      }
    } else {
      const { getSupabaseServerClient } = await import('@/lib/supabase/server')
      const supabase = await getSupabaseServerClient()
      const { data: profileData } = await supabase
        .from('profiles')
        .select('preferred_stores, store_category_assignments, shopping_style, preferred_delivery_service, grocery_preferences')
        .eq('id', userId)
        .single()

      if (profileData) {
        const stores = profileData.preferred_stores || profileData.grocery_preferences?.stores || []
  ...
  try {
    ...
    if (members.length > 0) {
      ...
      members.forEach(m => {
        ...
        if (m.dietary_restrictions?.length) parts.push(`| Diet: ${m.dietary_restrictions.join(', ')}`)
        if (m.is_picky_eater && m.picky_issues?.length) parts.push(`| Picky: ${m.picky_issues.join(', ')}`)
      ...
      members.forEach(m => {
        if (m.track_macros && m.macro_carbs_g != null) {
          householdLines.push(`${m.name} has a daily carb limit of ${m.macro_carbs_g}g — never suggest meals that would push this member's daily carbs over this limit.`)
        }
        if (m.age != null && m.age < 13) {
          householdLines.push(`${m.name} is a child aged ${m.age} — prioritize kid-friendly meals with appropriate portion sizes and avoid choking hazards or overly spicy ingredients.`)
        }
      })
    }
  } catch {
  ...
  let tasteLines = []
  try {
    const { getTasteProfile, buildTasteProfilePrompt, getFrequentWebsites } = await import('@/lib/dal/taste-profile')
    ...
    if (frequentSites.length > 0) {
      tasteLines.push(`This user frequently imports recipes from ${frequentSites.join(', ')}. When searching for recipes online, prioritize searching those websites first before branching out to other sources.`)
    }
  } catch {
    ...
    if (!isMockMode()) {
      ...
      if (adjustments?.length > 0) {
        ...
        adjustments.forEach(a => {
          adjustmentLines.push(`- ${a.member_name} on ${a.day} (${a.meal_type}): ${a.adjustment_type}${a.notes ? ' — ' + a.notes : ''}`)
        })
      }
    }
  } catch {
  ...
  try {
    const { getHouseholdFaithPractices, getAllMemberFaithPractices } = await import('@/lib/dal/faith-practices')
    const { FAITH_PROMPT_INSTRUCTIONS, FAITH_GENERAL_INSTRUCTION, FAITH_PRACTICE_OPTIONS } = await import('@/data/faith-practices')
      ...
      for (const member of memberFaith) {
        ...
        for (const practiceId of mf.individual_faith_practices) {
          ...
          if (practice.selectType === 'single' && practice.levelField) {
            ...
            if (mf.lds_no_coffee) faithLines.push(`  - ${member.name}: ${FAITH_PROMPT_INSTRUCTIONS.lds_no_coffee}`)
            if (mf.lds_no_alcohol) {
              faithLines.push(`  - ${member.name}: ${FAITH_PROMPT_INSTRUCTIONS.lds_no_alcohol}`)
              hasAlcoholRestriction = true
            }
            if (mf.lds_no_black_tea) faithLines.push(`  - ${member.name}: ${FAITH_PROMPT_INSTRUCTIONS.lds_no_black_tea}`)
  ...
  if (pantryItems.length > 0) {
    ...
    lines.push(
      '',
      'PANTRY CONTENTS (sorted by expiry date, soonest first):',
    )
