...
const TTS_PROVIDER = typeof window !== 'undefined'
  ? (process.env.NEXT_PUBLIC_TTS_PROVIDER || 'webspeech')
  : 'webspeech'
...
function pickVoice() {
  ...
  const female = englishVoices.find(v =>
    /female|woman/i.test(v.name) || /Samantha|Karen|Zira|Moira|Fiona/i.test(v.name)
  )
