...
export function buildWeekScaffold(rows = [], weekOffset = 0) {
  ...
  return DAYS.map(({ day, dayOfWeek }) => {
    const meals = MEAL_TYPES.map((type) => {
      const row = (rows || []).find((r) => r.day_of_week === dayOfWeek && r.meal_type === type);
      return {
        type,
...
export async function getWeeklyMeals(userId, weekOffset = 0) {
  ...
  const { data, error } = await supabase
    .from('meal_slots')
    .select(
      '*, recipes(id, name, description, image_url, tags, prep_time_minutes, cook_time_minutes, servings, ingredients, instructions)',
    )
...
export async function getSwapSuggestions(userId) {
  ...
  const { getSupabaseServerClient } = await import('@/lib/supabase/server');
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.from('recipes').select('*').limit(4);
