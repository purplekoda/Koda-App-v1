...
export async function analyzeAndUpdateTasteProfile(userId, recipe) {
  try {
    ...
    if (totalTime > 0 && totalTime <= 30 && !foundStyles.includes('quick'))
      foundStyles.push('quick');
    ...
    if (tags.includes('vegetarian') || tags.includes('vegan'))
      foundProteins.push(tags.find((t) => t === 'vegetarian' || t === 'vegan'));
...
export function buildTasteProfilePrompt(profile) {
  ...
  if (profile.avoided_ingredients?.length) {
    lines.push(
      `AVOID these ingredients (user has removed or skipped them): ${profile.avoided_ingredients.join(', ')}`,
    );
  }
