...
import {
  getMockProfile,
  saveMockProfile,
  getMockGroceryPreferences,
  saveMockGroceryPreferences,
  getMockRecipeCardSettings,
  saveMockRecipeCardSettings,
  getMockDashboardSections,
  saveMockDashboardSections,
} from './mock-store';
...
export async function getProfile(userId) {
  ...
  const { data, error } = await supabase
    .from('profiles')
    .select(
      'display_name, family_name, location, preferred_store, shopping_style, preferred_delivery_service',
    )
    .eq('id', userId)
    .single();
