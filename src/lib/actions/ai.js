...
const VALID_CONTEXTS = ['dashboard', 'meals', 'grocery', 'pantry', 'recipes', 'events', 'general'];
...
async function handleSwapIntent(user, cleanPrompt, cleanContext, intent) {
  ...
  const updatedHistory = [...history, { role: 'user', parts: [{ text: cleanPrompt }] }];
  ...
  const finalHistory = [...updatedHistory, { role: 'model', parts: [{ text: result.message }] }];
...
export async function confirmMealEditAction(cardData) {
  try {
    ...
    if (suggestion.ingredients_needed?.length > 0) {
      ...
      const { error: groceryError } = await supabase.from('grocery_items').insert(items);
