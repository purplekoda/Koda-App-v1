...
export function validateCookingPreferences(body) {
  ...
  if (
    body.macro_protein !== undefined &&
    body.macro_protein !== null &&
    body.macro_protein !== ''
  ) {
  ...
  if (
    body.macro_calories !== undefined &&
    body.macro_calories !== null &&
    body.macro_calories !== ''
  ) {
  ...
  if (
    body.weekly_budget !== undefined &&
    body.weekly_budget !== null &&
    body.weekly_budget !== ''
  ) {
...
export function validateHouseholdMember(body) {
  ...
  if (Array.isArray(body.picky_issues)) {
    data.picky_issues = body.picky_issues
      .slice(0, 20)
  ...
  if (Array.isArray(body.dietary_restrictions)) {
    data.dietary_restrictions = body.dietary_restrictions
      .slice(0, 20)
  ...
  if (data.track_macros) {
    if (body.macro_calories != null)
      data.macro_calories = sanitizeInteger(body.macro_calories, 0, 10000);
    if (body.macro_protein_g != null)
      data.macro_protein_g = sanitizeInteger(body.macro_protein_g, 0, 500);
    if (body.macro_carbs_g != null)
      data.macro_carbs_g = sanitizeInteger(body.macro_carbs_g, 0, 800);
...
export function validateCookTimeStep(body) {
  ...
  const time = sanitizeEnum(body.cook_time_preference, [
    'under_20',
    '20_to_40',
    'up_to_60',
    'depends',
...
export function validateBudgetStep(body) {
  ...
  if (body.shopping_style) {
    const style = sanitizeEnum(body.shopping_style, [
      'deal_hunter',
      'convenience',
      'pickup_only',
      'delivery_preferred',
  ...
  if (body.preferred_delivery_service) {
    const svc = sanitizeEnum(body.preferred_delivery_service, [
      'instacart',
      'doordash',
      'walmart_plus',
      'amazon_fresh',
