...
import { switchToManualAction, switchToVoiceAction } from './partial-data-actions';
    ...
    case PHASE.WELCOME:
      return (
        <PageWrapper>
          ...
          <WelcomeStep onStart={() => setPhase(PHASE.MODE_SELECT)} onSkip={handleSkip} />
        </PageWrapper>
      );
          ...
          <CompletionStep
            data={
              completionData
                ? {
                    household_size:
                      completionData.household?.household_size ||
                      completionData.household_size ||
                      0,
                    cook_time_preference:
                      completionData.schedule?.cook_time_preference ||
                      completionData.cook_time_preference ||
                      null,
                    meal_plan_days:
                      completionData.schedule?.meal_plan_days ||
                      completionData.meal_plan_days ||
                      [],
                    cuisines:
                      completionData.food_preferences?.cuisines || completionData.cuisines || [],
                    adventurousness:
                      completionData.food_preferences?.adventurousness ||
                      completionData.adventurousness ||
                      null,
                    meal_prep_style:
                      completionData.food_preferences?.meal_prep_style ||
                      completionData.meal_prep_style ||
                      null,
                    cooking_frustrations:
                      completionData.food_preferences?.cooking_frustrations ||
                      completionData.cooking_frustrations ||
                      [],
                    weekly_budget:
                      completionData.budget?.weekly_budget || completionData.weekly_budget || 0,
                    shopping_style:
                      completionData.budget?.shopping_style ||
                      completionData.shopping_style ||
                      null,
                    favorite_stores:
                      completionData.budget?.preferred_stores ||
                      completionData.preferred_stores ||
                      [],
                    health_goals:
                      completionData.health?.health_goals || completionData.health_goals || [],
                  }
                : {}
            }
            onStartPlanning={() => {
