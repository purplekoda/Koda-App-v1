...
import { createRecipe, updateRecipe, deleteRecipe, getRecipeById } from '@/lib/dal/recipes';
import {
  createCollection,
  updateCollection,
  deleteCollection,
...
export async function createRecipeAction(formData) {
  try {
    ...
    import('@/lib/gemini')
      ...
      .then((nutrition) =>
        updateRecipe(user.id, recipe.id, { nutrition, nutrition_is_estimated: true }),
      )
      .catch(() => {});
...
export async function getRecipeGenerationContextAction() {
  try {
    ...
    const { getCookingPreferences, getDietaryRestrictions } = await import(
      '@/lib/dal/cooking-preferences'
    );
...
async function extractRecipeImage(html, pageUrl, userId) {
  try {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from(RECIPE_IMAGE_BUCKET).getPublicUrl(storagePath);
...
export async function importRecipeFromUrlAction(url) {
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      return fail(
        "We couldn't read this page \u2014 it may require a login. Try copying the recipe text directly instead.",
      );
    }
    if (!res.ok) {
...
export async function importRecipeFromPhotoAction(formData) {
  try {
    ...
    const validation = validateRecipe(raw);
    if (!validation.valid)
      return fail('Could not identify a recipe from this photo. Try a clearer image.');
    let raw
    try {
      const { identifyDishFromPhoto } = await import('@/lib/gemini')
...
export async function generateRecipeIdeasAction(mode) {
  try {
    ...
    const [preferences, dietaryRestrictions, pantryItems, { systemPromptPrefix }] =
      await Promise.all([
        getCookingPreferences(user.id).catch(() => null),
        getDietaryRestrictions(user.id).catch(() => []),
        getPantryItems(user.id).catch(() => []),
...
export async function generateRecipeImageAction(recipeName, description) {
  try {
    ...
    if (uploadError) {
      ...
      return fail(
        'Generated photo but could not save it. Check that the recipe-images storage bucket exists.',
      );
    }
...
const MOCK_WEB_SEARCH_RESULTS = [
  {
    ...
    description:
      'Crispy breaded chicken cutlets topped with marinara and melted mozzarella. A beloved Italian-American comfort food classic perfect for weeknight dinners.',
    rating: 4.8,
    review_count: 3241,
