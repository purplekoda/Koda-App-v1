...
const ControlButton = styled.button`
  ...
  background: ${({ theme, $active }) => ($active ? theme.colors.tealLight : theme.colors.surface)};
  color: ${({ theme, $active }) => ($active ? theme.colors.tealDark : theme.colors.textPrimary)};
  cursor: pointer;
  min-height: 36px;
...
const Chip = styled.button`
  ...
  border: 0.5px solid
    ${({ theme, $active }) => ($active ? theme.colors.tealDark : theme.colors.border)};
  background: ${({ theme, $active }) => ($active ? theme.colors.tealDark : theme.colors.surface)};
...
const CardThumb = styled.img`
  ...
  ${({ $position }) =>
    $position === 'top' ? 'width: 100%; height: 120px;' : 'width: 100px; align-self: stretch;'}
`;
...
const ChipCount = styled.span`
  font-size: 11px;
  font-weight: 600;
  background: ${({ theme, $active }) => ($active ? theme.colors.teal : theme.colors.borderLight)};
...
const ColCheck = styled.span`
  ...
  background: ${({ theme, $checked }) => ($checked ? theme.colors.teal : 'transparent')};
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 11px;
...
export default function RecipesPageClient({
  initialRecipes,
  faithPractices,
  cardSettings: initialCardSettings,
  initialCollections,
  initialCollectionLinks,
}) {
      ...
      .then((result) => {
        ...
        if (result.success && result.data) {
          ...
          if (preferences) {
            ...
            setPrefServings(
              preferences.serving_size != null ? String(preferences.serving_size) : '',
            );
  ...
  const contextLoading =
    (promptOpen || wizardStep === 'choose' || wizardStep === 'custom') && contextData === null;
  ...
  const hasGeminiRecipe = useMemo(() => recipes.some((r) => r.source === 'gemini'), [recipes]);
    ...
    const list = base.filter((r) => {
      ...
      if (macroGoals.length > 0 || macroCalorie || macroProtein) {
        ...
        if (n) {
          ...
          if (
            macroCalorie &&
            MACRO_CALORIE_RANGES[macroCalorie] &&
            !MACRO_CALORIE_RANGES[macroCalorie](n)
          )
            return false;
          if (
            macroProtein &&
            MACRO_PROTEIN_RANGES[macroProtein] &&
            !MACRO_PROTEIN_RANGES[macroProtein](n)
          )
            return false;
        }
  ...
  const activeFilterItems = useMemo(() => {
    ...
    for (const goal of macroGoals) {
      ...
      items.push({
        label: opt?.label || goal,
        remove: () => setMacroGoals((p) => p.filter((g) => g !== goal)),
      });
    }
    if (macroCalorie) {
    ...
    for (const ing of activeIngredients) {
      items.push({
        label: ing,
        remove: () => setActiveIngredients((p) => p.filter((i) => i !== ing)),
      });
    }
  ...
  async function handleToggleRecipeCollection(recipeId, collectionId) {
    ...
    if (isIn) {
      ...
      if (result.success) {
        setCollectionLinks((prev) => [
          ...prev,
          { recipe_id: recipeId, collection_id: collectionId },
        ]);
      }
    }
  }
  ...
  return (
    <>
      <PageHeader>
        ...
        <HeaderActions>
          <SettingsIconLink
            href="/settings/recipe-card?from=recipes"
            title="Recipe card settings"
            aria-label="Recipe card settings"
          >
            {'\u2699\uFE0F'}
          </SettingsIconLink>
          <CreateMenuWrap ref={createMenuRef}>
            <AddButton onClick={() => setCreateMenuOpen((p) => !p)}>+ Create recipe</AddButton>
            {createMenuOpen && (
              <CreateMenuDropdown>
                <CreateMenuItem
                  type="button"
              ...
              {openPopover === 'filters' && (
                <FilterPanel role="dialog" aria-label="Filter recipes">
                  ...
                  >
                    ...
                    <CategoryChevron $open={openFilterCategory === 'TIME'}>
                      {'\u25BE'}
                    </CategoryChevron>
                  </FilterCategoryRow>
                  ...
                  >
                    <CategoryLabel>
                      ...
                      {categoryCounts['MEAL TYPE'] > 0 && (
                        <CategoryCount>
                          {'\u00B7'} {categoryCounts['MEAL TYPE']}
                        </CategoryCount>
                      )}
                    </CategoryLabel>
                    <CategoryChevron $open={openFilterCategory === 'MEAL TYPE'}>
                      {'\u25BE'}
                    </CategoryChevron>
                  </FilterCategoryRow>
                  ...
                  >
                    <CategoryLabel>
                      ...
                      {categoryCounts['MACROS'] > 0 && (
                        <CategoryCount>
                          {'\u00B7'} {categoryCounts['MACROS']}
                        </CategoryCount>
                      )}
                    </CategoryLabel>
                    <CategoryChevron $open={openFilterCategory === 'MACROS'}>
                      {'\u25BE'}
                    </CategoryChevron>
                  </FilterCategoryRow>
                      ...
                      <ChipRow>
                        {MACROS_CALORIE_OPTIONS.map((opt) => (
                          <Chip
                            ...
                            onClick={() =>
                              setMacroCalorie(macroCalorie === opt.value ? null : opt.value)
                            }
                      ...
                      <ChipRow>
                        {MACROS_PROTEIN_OPTIONS.map((opt) => (
                          <Chip
                            ...
                            onClick={() =>
                              setMacroProtein(macroProtein === opt.value ? null : opt.value)
                            }
                  ...
                  >
                    <CategoryLabel>
                      ...
                      {categoryCounts['CUISINE TYPE'] > 0 && (
                        <CategoryCount>
                          {'\u00B7'} {categoryCounts['CUISINE TYPE']}
                        </CategoryCount>
                      )}
                    </CategoryLabel>
                    <CategoryChevron $open={openFilterCategory === 'CUISINE TYPE'}>
                      {'\u25BE'}
                    </CategoryChevron>
                  </FilterCategoryRow>
                      ...
                      >
                        <CategoryLabel>
                          ...
                          {categoryCounts['INGREDIENTS'] > 0 && (
                            <CategoryCount>
                              {'\u00B7'} {categoryCounts['INGREDIENTS']}
                            </CategoryCount>
                          )}
                        </CategoryLabel>
                        <CategoryChevron $open={openFilterCategory === 'INGREDIENTS'}>
                          {'\u25BE'}
                        </CategoryChevron>
                      </FilterCategoryRow>
                  ...
                  {allTags.length > 0 && (
                    <>
                      ...
                      >
                        ...
                        <CategoryChevron $open={openFilterCategory === 'TAGS'}>
                          {'\u25BE'}
                        </CategoryChevron>
                      </FilterCategoryRow>
                  ...
                  {hasGeminiRecipe && (
                    <>
                      ...
                      >
                        <CategoryLabel>
                          SOURCE
                          {sourceFilter !== 'all' && <CategoryCount>{'\u00B7'} 1</CategoryCount>}
                        </CategoryLabel>
                        <CategoryChevron $open={openFilterCategory === 'SOURCE'}>
                          {'\u25BE'}
                        </CategoryChevron>
                      </FilterCategoryRow>
              ...
              <CardWrapper key={recipe.id} $photoTop={photoTop}>
                <Card href={`/recipes/${recipe.id}`} $photoTop={photoTop}>
                  ...
                  <CardBody>
                    <CardName>
                      {hasFaithConflict && (
                        <span title="Faith-based dietary conflict" style={{ marginRight: 4 }}>
                          {'\u26A0\uFE0F'}
                        </span>
                      )}
                      {recipe.name}
                    </CardName>
                    {cs.show_description && recipe.description && (
                      <CardDescription>{recipe.description}</CardDescription>
                    )}
                    <Meta>
                      {cs.show_cook_time && time && (
                        <span>
                          {'\u23F1 '}
                          {time}
                        </span>
                      )}
                      {cs.show_servings && recipe.servings && (
                        <span>
                          {'\uD83D\uDC65 '}
                          {recipe.servings}
                        </span>
                    ...
                    {cs.show_difficulty && recipe.difficulty && (
                      <div>
                        <CardDifficultyBadge>{recipe.difficulty}</CardDifficultyBadge>
                      </div>
                    )}
                      ...
                      <>
                        ...
                        <ColMenuPanel>
                          {collections.map((col) => {
                            const isIn = collectionLinks.some(
                              (l) =>
                                String(l.recipe_id) === String(recipe.id) &&
                                l.collection_id === col.id,
                            );
                            return (
                              <ColMenuItem
                                key={col.id}
                ...
                <>
                  <WizardModeGrid>
                    ...
                    >
                      ...
                      <ModeCardMeta>Tell Koda what you want to cook</ModeCardMeta>
                    </ModeCard>
                  </WizardModeGrid>
                </>
              )}
                  ...
                  <ContextSection>
                    {contextData.pantryItems?.length > 0 && (
                      <PantryToggleRow>
                        ...
                        <PantryToggleMeta>
                          {contextData.pantryItems.length} item
                          {contextData.pantryItems.length !== 1 ? 's' : ''}
                          {(() => {
                            const exp = contextData.pantryItems.filter(
                              (i) => i.freshness === 'expiring',
                            ).length;
                    ...
                    {contextData.dietaryRestrictions?.length > 0 && (
                      <DietaryRow>Dietary: {contextData.dietaryRestrictions.join(', ')}</DietaryRow>
                    )}
                            ...
                            <PrefsChipRow>
                              {[
                                ...
                                <PrefChip
                                  ...
                                  onClick={() =>
                                    setPrefTime(prefTime === opt.value ? null : opt.value)
                                  }
                                >
                                  {opt.label}
                                </PrefChip>
      ...
      {scanOpen && (
        <Modal title="Scan a recipe" onClose={closeScan}>
          <PromptHint>
            Take photos of a recipe — from a cookbook, handwritten card, or screen. Add up to 6
            images.
          </PromptHint>
          {scanFiles.length === 0 ? (
            <ScanDropZone>
          ...
          <PromptActions>
            ...
            <PromptPrimary onClick={handleScan} disabled={isScanning || scanFiles.length === 0}>
              {isScanning
                ? scanPhase || 'Scanning\u2026'
                : `Scan ${scanFiles.length > 0 ? `(${scanFiles.length} photo${scanFiles.length !== 1 ? 's' : ''})` : ''}`}
            </PromptPrimary>
          </PromptActions>
        </Modal>
      )}
      ...
      {urlOpen && (
        <Modal title="Import recipe" onClose={() => setUrlOpen(false)}>
          ...
          <SearchInput
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && !isImporting && urlText.trim() && handleImportUrl()
            }
          ...
          />
          {isImporting && (
            <PromptHint style={{ marginTop: 8 }}>{'\u2728'} Reading the recipe\u2026</PromptHint>
          )}
      ...
      {importReviewOpen && importReviewData && (
        <Modal
          title={
            importReviewData.imported_from
              ? `Imported from ${importReviewData.imported_from}`
      ...
      {webSearchOpen && (
        <Modal
          title={
            webSearchStep === 'detail'
              ? webSearchSelected?.name || 'Recipe detail'
              : 'Search the web'
          }
        ...
        >
          {webSearchStep === 'input' && (
            <>
              ...
              {isWebSearching && (
                <WebSearchHint style={{ marginTop: '8px' }}>Searching the web…</WebSearchHint>
              )}
              <PromptActions>
                ...
                {webSearchResults.map((r, i) => {
                  ...
                  return (
                    <WebSearchResultCard
                      ...
                      type="button"
                      onClick={() => {
                        setWebSearchSelected(r);
                        setWebSearchStep('detail');
                      }}
                    >
                      {r.image_url && (
                      ...
                      <WebSearchResultBody>
                        ...
                        {(stars || r.review_count) && (
                          <WebSearchResultRatingRow>
                            ...
                            {r.rating && (
                              <WebSearchResultMeta>{r.rating.toFixed(1)}</WebSearchResultMeta>
                            )}
                            {r.review_count && (
                              <WebSearchResultMeta>
                                {formatReviews(r.review_count)}
                              </WebSearchResultMeta>
                            )}
                          </WebSearchResultRatingRow>
                        )}
                        ...
                        {r.description && (
                          <WebSearchResultDesc>{r.description}</WebSearchResultDesc>
                        )}
                      </WebSearchResultBody>
                    </WebSearchResultCard>
                  );
          ...
          {webSearchStep === 'detail' && webSearchSelected && (
            <>
              ...
              <WebSearchDetailMetaRow>
                {webSearchSelected.rating && (
                  <WebSearchDetailMetaChip>
                    ...
                    {webSearchSelected.review_count
                      ? ` (${formatReviews(webSearchSelected.review_count)})`
                      : ''}
                  </WebSearchDetailMetaChip>
                )}
                {(Number(webSearchSelected.prep_time_minutes) || 0) +
                  (Number(webSearchSelected.cook_time_minutes) || 0) >
                  0 && (
                  <WebSearchDetailMetaChip>
                    {'\u23F1 '}
                    {formatMinutes(
                ...
                {webSearchSelected.servings && (
                  <WebSearchDetailMetaChip>
                    {webSearchSelected.servings} servings
                  </WebSearchDetailMetaChip>
                )}
              </WebSearchDetailMetaRow>
              ...
              {webSearchSelected.source_url && (
                <WebSearchSourceLink
                  href={webSearchSelected.source_url}
                  target="_blank"
              ...
              {webSearchError && (
                <PromptError role="alert" style={{ marginTop: '12px' }}>
                  {webSearchError}
                </PromptError>
              )}
              ...
              <PromptActions>
                <PromptSecondary
                  onClick={() => setWebSearchStep('results')}
                  disabled={isSavingWebRecipe}
                >
                  Back
                </PromptSecondary>
