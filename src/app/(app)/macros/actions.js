...
import {
  getMacroMembers,
  addMacroExtra,
  updateMacroExtra as dalUpdateExtra,
  getMacroExtrasForWeek,
  getMacroExtrasForMonth,
} from '@/lib/dal/macros';
...
function getFallbackInsight(member) {
  const avgProtein =
    member.weeklyActuals.reduce((sum, d) => sum + d.protein, 0) / member.weeklyActuals.length;
