...
export async function addPantryItemAction(formData) {
  try {
    ...
    const unit = sanitizeEnum(formData.get?.('unit') ?? formData.unit, [...PANTRY_UNITS, '']);
    ...
    const category =
      sanitizeEnum(formData.get?.('category') ?? formData.category, PANTRY_CATEGORIES) || 'Other';
...
export async function commitScanToPantryAction(scanItems, manualItems, deleteExisting) {
  try {
    ...
    const cleanScan = (Array.isArray(scanItems) ? scanItems : [])
      .map((item) => ({
    ...
    const cleanManual = (Array.isArray(manualItems) ? manualItems : [])
      .map((item) => ({
        name: sanitizeString(String(item.name ?? ''), 200),
        category: sanitizeEnum(item.category, PANTRY_CATEGORIES) || 'Other',
      }))
