...
const ItemCard = styled.div`
  ...
  border: 0.5px solid ${({
    ...
    $status === 'expired' ? theme.colors.coral :
...
const MetaChip = styled.span`
  ...
  background: ${({ theme, $variant }) =>
    $variant === 'expired' ? theme.colors.coralLight :
    $variant === 'expiring' ? '#FEF3C7' :
    theme.colors.borderLight
  };
  color: ${({ theme, $variant }) =>
    $variant === 'expired' ? theme.colors.coral :
    $variant === 'expiring' ? '#92400E' :
...
const ScanBanner = styled.div`
  ...
  background: ${({ theme }) => theme.colors.surface};
  border: 0.5px solid ${({ $stale, theme }) =>
    $stale ? '#F59E0B' : theme.colors.borderLight};
  border-left: 3px solid ${({ $stale, theme }) =>
    $stale ? '#F59E0B' : theme.colors.teal};
  border-radius: ${({ theme }) => theme.radii.lg};
  margin-bottom: ${({ theme }) => theme.spacing.xl};
  flex-wrap: wrap;
`
...
export default function PantryPageClient({
  ...
  function handleSaveScanItems() {
    ...
    startSaveScanTransition(async () => {
      const result = await commitScanToPantryAction(scanEditItems, scanManualAdds, shouldDeleteManual)
      if (result.success) {
        setScanScreen('idle')
        setScanEditItems([])
        setScanManualAdds([])
  ...
  const scanDate = scanItems.length > 0
    ? formatScanDate(scanItems.reduce((latest, i) =>
        !latest || i.created_at > latest ? i.created_at : latest, null))
    : null

  function renderItem(item) {
    const status = getExpiryStatus(item.expiry_date)
    const expiryLabel = item.expiry_date
      ? status === 'expired' ? `Expired ${formatExpiry(item.expiry_date)}`
      : status === 'expiring' ? `Expires ${formatExpiry(item.expiry_date)}`
      ...
      <ItemCard key={item.id} $status={status} $depleted={item.is_depleted}>
        <ItemMain>
          ...
          <ItemMeta>
            ...
            {expiryLabel && (
              <MetaChip $variant={status === 'expired' ? 'expired' : status === 'expiring' ? 'expiring' : undefined}>
                {expiryLabel}
              </MetaChip>
            )}
          </ItemMeta>
  ...
  return (
    <>
      <PageTitle>
        ...
        <MainSubtitle>Manage your ingredients and staples — Koda uses this for meal planning.</MainSubtitle>
      </PageTitle>
      ...
      {scanScreen === 'results' && (
        <>
          <ScanResultsHeader>
            <ScanResultsTitle>
              Review {scanEditItems.length + scanManualAdds.length} detected item{scanEditItems.length + scanManualAdds.length !== 1 ? 's' : ''}
            </ScanResultsTitle>
            <RescanLink onClick={() => setScanScreen('scanning')}>
              Scan again
            </RescanLink>
          </ScanResultsHeader>
          ...
          <ScanEditList>
            {scanEditItems.map(item => (
              <ScanEditRow key={item.id}>
                <ItemMain>
                  ...
                  <ItemMeta>
                    <MetaChip>{item.category}</MetaChip>
                    {item.freshness === 'expiring' && <MetaChip $variant="expiring">Expiring soon</MetaChip>}
                    {item.freshness === 'low' && <MetaChip $variant="expired">Running low</MetaChip>}
                  </ItemMeta>
                </ItemMain>
            ...
            {scanManualAdds.map(item => (
              <ScanEditRow key={item.id}>
                ...
                <DangerIconButton
                  title="Remove"
                  onClick={() => setScanManualAdds(prev => prev.filter(i => i.id !== item.id))}
                >
                  {'\u2715'}
                </DangerIconButton>
              </ScanEditRow>
          ...
          <ManualAddSection>
            ...
            <ManualAddRow>
              <Input
                ...
                onKeyDown={e => { if (e.key === 'Enter') handleAddManualScanItem() }}
      ...
      {items.length > 0 && (
        <LegendRow>
          <LegendItem>
            <LegendDot $color={({ theme }) => theme?.colors?.coral || '#EF4444'} style={{ background: '#EF4444' }} />
            Expired
          </LegendItem>
          <LegendItem>
      ...
      {items.length === 0 ? (
        <EmptyState>
          ...
          <p>Add ingredients you have on hand. Koda will always check here first when planning meals.</p>
        </EmptyState>
      ) : (
        <ItemList>
            ...
            {!preScanShowList ? (
              <>
                <PreScanQuestion>
                  ...
                  {items.filter(i => i.source === 'manual').length !== 1 ? 's are' : ' is'} already in your pantry.
                  Keep {items.filter(i => i.source === 'manual').length !== 1 ? 'them' : 'it'} after this scan?
                </PreScanQuestion>
                <ModalActions>
                  <SecondaryButton onClick={handlePreScanNo} disabled={isPending}>
                    No, remove {items.filter(i => i.source === 'manual').length !== 1 ? 'them' : 'it'}
                  </SecondaryButton>
              ...
              <>
                ...
                <PreScanItemList>
                  ...
                  {items.filter(i => i.source === 'manual').length === 0 && (
                    <PreScanQuestion style={{ marginBottom: 0 }}>All manual items removed.</PreScanQuestion>
                  )}
                </PreScanItemList>
                <ModalActions>
                  <SecondaryButton onClick={() => setPreScanOpen(false)}>
                    Cancel
                  </SecondaryButton>
