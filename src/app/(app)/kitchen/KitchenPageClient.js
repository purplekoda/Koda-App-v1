...
export default function KitchenPageClient({
  ...
  const [screen, setScreen] = useState(() => initialScreen(initialPantryItems, initialLastScan));
  ...
  return (
    <>
      ...
      {screen === 'entry' && <ScanEntry lastScan={lastScan} onStartScan={handleStartScan} />}

      {screen === 'scanning' && <ScanUpload onScanComplete={handleScanComplete} />}
      ...
      {screen === 'results' && (
        <>
          ...
          <RescanRow>
            <RescanButton onClick={handleRescan}>Scan again</RescanButton>
          </RescanRow>
