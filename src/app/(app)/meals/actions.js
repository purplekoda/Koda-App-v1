...
export async function swapMeal(day, mealType, newMealName) {
  try {
    ...
    const cleanType = sanitizeEnum(mealType, [
      'breakfast',
      'lunch',
      'dinner',
      'sides',
    ...
    const mealPlanId = await import('@/lib/dal/meals').then((m) =>
      m.getOrCreateMealPlan(user.id, supabase),
    );
...
export async function assignRecipeToSlot(day, mealType, recipeId) {
  try {
    ...
    if (recipeError || !recipe) {
      console.error(
        '[assignRecipeToSlot recipe lookup error]',
        recipeError,
        'recipeId=',
        cleanRecipeId,
        'userId=',
    ...
    if (error) {
      console.error('[assignRecipeToSlot UPSERT error]', error, 'payload:', {
        day: cleanDay,
        type: cleanType,
 ...
 */
export async function aiFillWeekAction(
  userInstructions,
  modes = ['fill-for-me'],
  weekOffset = 0,
  filters = {},
  ...
  try {
    ...
    function buildModeInstruction(selectedModes, hasUserRecipes) {
      ...
      if (useMyRecipes && !useWebSearch && !useFillForMe) {
        lines.push(
          "PLANNING MODE: Use saved recipes only. Fill empty slots exclusively from the user's Recipe Box. Do not suggest new or unknown recipes.",
        );
      } else if (useWebSearch && !useMyRecipes && !useFillForMe) {
        lines.push(
          'PLANNING MODE: Web search only. Suggest highly-rated, popular recipes for each slot. All suggestions should be well-known recipes with high community ratings.',
        );
      } else if (useFillForMe && !useMyRecipes && !useWebSearch) {
        lines.push(
          "PLANNING MODE: Mixed. Combine the user's saved recipes with new AI-generated ideas to build a balanced, full week.",
        );
      } else if (useMyRecipes && useWebSearch && !useFillForMe) {
        lines.push(
          'PLANNING MODE: Blend saved recipes and web-found recipes. Fill some slots from the Recipe Box and search online for the rest, prioritizing pantry fit.',
        );
      } else if (useMyRecipes && useFillForMe && !useWebSearch) {
        lines.push(
          'PLANNING MODE: Saved recipes first, then AI ideas. Use saved recipes as the primary source and fill remaining gaps with AI-generated ideas. Skip web search.',
        );
      } else if (useWebSearch && useFillForMe && !useMyRecipes) {
        lines.push(
          'PLANNING MODE: Web search + AI ideas. Search online for highly-rated suggestions and supplement with AI-generated ideas for any remaining slots.',
        );
      } else {
        lines.push(
          'PLANNING MODE: Comprehensive. Use saved recipes first, fill gaps with online search results, and use AI-generated ideas for any remaining empty slots.',
        );
      }
      return lines.join('\n');
    }
    ...
    function buildFiltersInstruction(filters) {
      ...
      if (filters.weeklyBudget) {
        ...
        if (filters.stores?.length > 0) {
          ...
          lines.push(
            `The user shops at: ${storeNames}. When possible, suggest meals that use ingredients commonly on sale or competitively priced at these stores.`,
          );
        }
      }
    ...
    if (isMockMode()) {
      ...
      for (const slot of result.slots || []) {
        ...
        if (slot.is_saved_recipe) {
          const match = userRecipes.find(
            (r) => r.name.toLowerCase().trim() === (slot.meal_name || '').toLowerCase().trim(),
          );
    ...
    const userRecipes = (recipesResult.data || []).map((r) => ({
      name: r.name,
      tags: r.tags || [],
    }));
    ...
    const fullRecipes = (recipesResult.data || []).map((r) => ({
      id: r.id,
      name: r.name,
      tags: r.tags || [],
    }));
...
export async function searchRecipeSuggestionsAction(unlinkedSlots, weekOffset = 0) {
  try {
    ...
    if (isMockMode()) {
      const [pantry, meals] = await Promise.all([getMockPantry(), getMockMeals()]);
      pantryItems = pantry;
      weekMeals = meals.flatMap((dayObj) =>
        dayObj.meals
          .filter((m) => m.name)
          .map((m) => ({ day: dayObj.day, type: m.type, name: m.name })),
      );
    } else {
      const { getSupabaseServerClient } = await import('@/lib/supabase/server');
      const supabase = await getSupabaseServerClient();
      ...
      weekMeals = (slotsResult.data || [])
        .filter((s) => s.meal_name)
        .map((s) => ({
    ...
    const slotSuggestions = await searchRecipeSuggestionsForSlots(cleanSlots, {
      pantryItems,
      weekMeals,
    });
...
export async function surpriseMeAction(mealType) {
  try {
    ...
    const cleanMealType = mealType
      ? sanitizeEnum(mealType, ['breakfast', 'lunch', 'dinner'])
      : null;
    ...
    const { searchSurpriseMeRecipe } = await import('@/lib/gemini');
    const recipe = await searchSurpriseMeRecipe({
      mealPlanSystemPrompt: systemPromptPrefix,
      tasteProfilePrompt: tastePrompt,
...
export async function surpriseMeBatchAction(count, mealTypes) {
  try {
    ...
    const cleanTypes = (mealTypes || []).map((t) => sanitizeEnum(t, validTypes)).filter(Boolean);
    ...
    const { searchSurpriseMeRecipes } = await import('@/lib/gemini');
    const recipes = await searchSurpriseMeRecipes({
      mealPlanSystemPrompt: systemPromptPrefix,
      tasteProfilePrompt: tastePrompt,
  ...
  try {
    ...
    if (isMockMode()) {
      ...
      const filledDayTypes = new Set(
        (slotsResult.data || [])
          .filter((s) => s.custom_meal_name || s.recipe_id)
          .map((s) => `${s.day_of_week}:${s.meal_type}`),
      );
      const DAYS_LIST = [
        { day: 'Mon', dayOfWeek: 1 },
        { day: 'Tue', dayOfWeek: 2 },
    ...
    const slotResults = await searchWebRecipesForSlots(emptySlots, {
      pantryItems,
      dietaryRestrictions,
      preferences,
    });
...
export async function repeatMealAction(sourceDay, mealType, targetDays, weekOffset = 0) {
  try {
    ...
    if (!Array.isArray(targetDays) || targetDays.length === 0)
      return fail('No target days selected');
