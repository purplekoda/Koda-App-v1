...
const ItemRow = styled.div`
  ...
  border-radius: ${({ $open, theme }) =>
    $open
      ...
      : theme.radii.md};
  border-bottom-color: ${({ $open, theme }) =>
    $open ? 'transparent' : theme.colors.border};
  cursor: pointer;
  user-select: none;
`
...
export default function IngredientStoreAssignment({ recipe, groceryPreferences }) {
  ...
  if (!storeMap.length) {
    return (
      <Wrapper>
        ...
        <NoStoresHint>
          <a href="/settings/grocery-stores" style={{ color: '#1D9E75', textDecoration: 'underline' }}>
  ...
  const byStore = {}
  storeMap.forEach(s => { byStore[s.value] = [] })
  assignments.forEach((storeVal, i) => {
    const key = storeVal && byStore[storeVal] !== undefined ? storeVal : (fallback || storeMap[0]?.value)
  ...
  const visibleStores = filterStore === 'all'
    ? storeMap
    : storeMap.filter(s => s.value === filterStore)
  ...
  return (
    <Wrapper>
                  ...
                  {isOpen && (
                    <InlineDropdown>
                      {storeMap.map(opt => {
                        ...
                        const optDesc = GROCERY_STORES.find(g => g.value === opt.value)?.description || ''
      ...
      <SendBar>
        <SendInfo>
          <SendStrong>{listsReady} {listsReady === 1 ? 'list' : 'lists'} ready</SendStrong>
