...
const SectionRow = styled.div`
  ...
  background: ${({ $dragging, theme }) =>
    $dragging ? theme.colors.tealLight : 'transparent'};
  border-radius: ${({ $dragging, theme }) =>
    $dragging ? theme.radii.md : '0'};
...
function getSectionDef(sectionId) {
  return SECTION_DEFS.find(d => d.section_id === sectionId) || { label: sectionId, description: '' }
}
