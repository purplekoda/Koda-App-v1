...
const ToggleRow = styled.button`
  ...
  gap: ${({ theme }) => theme.spacing.md};
  padding: ${({ theme }) => theme.spacing.md};
  border: 1.5px solid ${({ $on, theme }) => ($on ? theme.colors.teal : theme.colors.border)};
  border-radius: ${({ theme }) => theme.radii.md};
  background: ${({ $on, theme }) =>
 ...
 */
export default function FaithPracticeCards({
  practices = [],
  practiceDetails = {},
  onChange,
  disabled,
  ...
  return (
    <CardList>
      <IntroText>
        Koda respects all dietary traditions. Select any that apply and we will honor them across
        all meal planning.
      </IntroText>
        ...
        return (
          <Card key={practice.id} $selected={selected}>
            ...
            {selected && practice.selectType !== 'boolean' && (
              <ExpandedPanel>
                {practice.introNote && <PanelNote>{practice.introNote}</PanelNote>}
                ...
                {practice.closingNote && <PanelNote>{practice.closingNote}</PanelNote>}
              </ExpandedPanel>
            )}
          </Card>
        );
      ...
      <BottomNote>
        For certified or strict observance levels we recommend verifying specific ingredient and
        preparation requirements with your religious authority as requirements can vary by community
        and interpretation.
      </BottomNote>
    </CardList>
  );
