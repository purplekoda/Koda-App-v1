...
import { PICKY_EATER_ISSUES, MEMBER_ALLERGY_OPTIONS, MEMBER_DIET_OPTIONS } from '@/data/onboarding-options'
...
export default function HouseholdMembersStep({ members, onChange, onNext, onBack, onSkip, isPending, embedded = false, userName, faithPractices, onChangeFaith, displayMode = false, activeMemberIdx = null }) {
      ...
      <MemberList>
        {members.map((member, idx) => {
          ...
          return (
            <CardComponent key={idx} $active={isActive}>
              <MemberHeader type="button" onClick={displayMode ? undefined : () => setExpandedIdx(isOpen ? null : idx)} style={displayMode ? { cursor: 'default' } : undefined}>
                <Avatar>{(member.name || '?')[0].toUpperCase()}</Avatar>
                <MemberInfo>
                  <MemberName>
                    {member.name || 'Unnamed'}
                    {member.is_owner && <Badge $color="#E0F2FE" $textColor="#0369A1" style={{ marginLeft: 6 }}>You</Badge>}
                  </MemberName>
                  <MemberMeta>
                    <Badge
                    ...
                    {(member.allergies || []).map(a => (
                      <Badge key={a} $color="#FEE2E2" $textColor="#991B1B">{a}</Badge>
                    ))}
                    {(member.dietary_restrictions || []).map(d => (
                      <Badge key={d} $color="#E0F2FE" $textColor="#0369A1">{d}</Badge>
                    ))}
                    {member.is_picky_eater && (
                      <Badge $color="#FEF3C7" $textColor="#92400E">Picky eater</Badge>
                    )}
                    {member.track_macros && (
                      <Badge $color="#D1FAE5" $textColor="#065F46">Macros</Badge>
                    )}
                  </MemberMeta>
                </MemberInfo>
              ...
              {isOpen && (
                <ExpandedContent>
                  ...
                  <div>
                    ...
                    {(member.allergies || []).filter(a => !MEMBER_ALLERGY_OPTIONS.includes(a)).length > 0 && (
                      <CustomTagRow>
                        {(member.allergies || []).filter(a => !MEMBER_ALLERGY_OPTIONS.includes(a)).map(tag => (
                          <CustomTag key={tag}>
                            {tag}
                            <RemoveTag type="button" onClick={() => toggleArrayItem(idx, 'allergies', tag)}>&times;</RemoveTag>
                          </CustomTag>
                        ))}
                      </CustomTagRow>
                    )}
                    <CustomRow>
                      <Input
                        ...
                        maxLength={60}
                      />
                      <AddBtn type="button" onClick={() => addCustomAllergy(idx)} disabled={!customAllergy.trim()}>Add</AddBtn>
                    </CustomRow>
                  </div>

                  <div>
                    <SectionLabel>Dietary restrictions</SectionLabel>
                    <SectionHint>Koda will make accommodations and suggest alternatives where possible.</SectionHint>
                    <ChipGrid>
                    ...
                    {(member.dietary_restrictions || []).filter(d => !MEMBER_DIET_OPTIONS.includes(d)).length > 0 && (
                      <CustomTagRow>
                        {(member.dietary_restrictions || []).filter(d => !MEMBER_DIET_OPTIONS.includes(d)).map(tag => (
                          <CustomTag key={tag}>
                            {tag}
                            <RemoveTag type="button" onClick={() => toggleArrayItem(idx, 'dietary_restrictions', tag)}>&times;</RemoveTag>
                          </CustomTag>
                        ))}
                      </CustomTagRow>
                    )}
                    <CustomRow>
                      <Input
                        ...
                        maxLength={60}
                      />
                      <AddBtn type="button" onClick={() => addCustomDiet(idx)} disabled={!customDiet.trim()}>Add</AddBtn>
                    </CustomRow>
                  </div>
                  ...
                  <div>
                    ...
                    {member.is_picky_eater && (
                      <PickyPanel>
                        ...
                        {(member.picky_issues || []).filter(p => !PICKY_EATER_ISSUES.includes(p)).length > 0 && (
                          <CustomTagRow>
                            {(member.picky_issues || []).filter(p => !PICKY_EATER_ISSUES.includes(p)).map(tag => (
                              <CustomTag key={tag}>
                                {tag}
                                <RemoveTag type="button" onClick={() => toggleArrayItem(idx, 'picky_issues', tag)}>&times;</RemoveTag>
                              </CustomTag>
                            ))}
                          </CustomTagRow>
                        )}
                        <CustomRow>
                          <Input
                            ...
                            maxLength={100}
                          />
                          <AddBtn type="button" onClick={() => addCustomNote(idx)} disabled={!customNote.trim()}>Add</AddBtn>
                        </CustomRow>
                        <AINote>Koda will use this to find meals the whole family can enjoy, suggesting easy swaps for picky eaters.</AINote>
                      </PickyPanel>
                    )}
                  </div>
                      ...
                      <MacrosPanel>
                        <MacroInputRow>
                          ...
                          <MacroInput
                            ...
                            onChange={e => updateMember(idx, { macro_calories: e.target.value === '' ? null : parseInt(e.target.value, 10) })}
                            min={0}
                            max={10000}
                          />
                        ...
                        <MacroInputRow>
                          ...
                          <MacroInput
                            ...
                            onChange={e => updateMember(idx, { macro_protein_g: e.target.value === '' ? null : parseInt(e.target.value, 10) })}
                            min={0}
                            max={500}
                          />
                        ...
                        <MacroInputRow>
                          ...
                          <MacroInput
                            ...
                            onChange={e => updateMember(idx, { macro_carbs_g: e.target.value === '' ? null : parseInt(e.target.value, 10) })}
                            min={0}
                            max={800}
                          />
                          <MacroUnit>g / day</MacroUnit>
                        ...
                        <MacroInputRow>
                          ...
                          <MacroInput
                            ...
                            onChange={e => updateMember(idx, { macro_fat_g: e.target.value === '' ? null : parseInt(e.target.value, 10) })}
                            min={0}
                            max={400}
                          />
                          <MacroUnit>g / day</MacroUnit>
                        </MacroInputRow>
                        <AINote>Koda will use these targets when building meal plans for this person.</AINote>
                      </MacrosPanel>
                    )}
                  </div>
                  ...
                  <div>
                    <FaithDivider>
                      ...
                      {faithPractices?.follows_faith_based_diet && member.individual_faith_practices?.follows_individual_faith_diet && (
                    ...
                    >
                      ...
                      <ToggleTrack $on={member.individual_faith_practices?.follows_individual_faith_diet}>
                        <ToggleKnob $on={member.individual_faith_practices?.follows_individual_faith_diet} />
                      </ToggleTrack>
                    </PickyToggle>
                    ...
                    {member.individual_faith_practices?.follows_individual_faith_diet && (
                      <PickyPanel>
                        ...
                        <FaithPracticeCards
                          practices={member.individual_faith_practices?.individual_faith_practices || []}
        ...
        {!displayMode && onChangeFaith && (
          <FaithSection>
            ...
            <FaithHint>Toggle on to apply to the entire household. Each member can still be adjusted individually above.</FaithHint>
            <FaithPracticeToggle
