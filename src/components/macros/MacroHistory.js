...
import {
  fetchWeeklyExtras,
  fetchMonthlyExtras,
  updateMacroExtraAction,
} from '@/app/(app)/macros/actions';
...
export default function MacroHistory({ memberId, memberName, targets }) {
  ...
  async function saveEdit() {
    ...
    if (result.success) {
      setWeeklyExtras((prev) =>
        prev.map((e) =>
          e.id === editingExtra.id ? { ...e, ...editValues, edited_by_user: true } : e,
        ),
      );
    }
    setSavingEdit(false);
    ...
    return (
      <>
              ...
              {expanded &&
                dayExtras.map((extra) => (
                  <ExtraItem key={extra.id}>
                    ...
                    {!extra.is_locked && <EditBtn onClick={() => openEdit(extra)}>Edit</EditBtn>}
                  </ExtraItem>
                ))}
            </DayRow>
          );
        ...
        {weeklyExtras.length === 0 && (
          <EmptyState>
            No extras logged this week. Use the &quot;Add food or snack&quot; button on the Today
            tab to get started.
          </EmptyState>
        )}
      </>
  ...
  function renderMonthly() {
    ...
    return (
      <>
        ...
        <CalendarGrid>
          {DAY_NAMES.map((d) => (
            <CalendarHeader key={d}>{d}</CalendarHeader>
          ))}
          ...
          <DayDetail>
            ...
            {byDay[selectedMonthDay].map((extra) => (
              <DayDetailRow key={extra.id}>
                <span>
                  {extra.food_name} ({MEAL_TIME_LABELS[extra.meal_time] || extra.meal_time})
                </span>
                <span>
                  {extra.calories} cal · {extra.protein}g P · {extra.carbs}g C · {extra.fat}g F
                </span>
              </DayDetailRow>
            ))}
          </DayDetail>
  ...
  return (
    <Wrapper>
      <ToggleRow>
        <ToggleBtn $active={mode === 'weekly'} onClick={() => setMode('weekly')}>
          Weekly
        </ToggleBtn>
        <ToggleBtn $active={mode === 'monthly'} onClick={() => setMode('monthly')}>
          Monthly
        </ToggleBtn>
      </ToggleRow>
