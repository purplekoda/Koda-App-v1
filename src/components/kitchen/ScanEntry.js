...
export default function ScanEntry({ lastScan, onStartScan }) {
  ...
  return (
    <Card $isStale={isStale}>
      {isStale && <StaleBadge>{daysAgo} days since your last scan</StaleBadge>}
      <Icon>{isStale ? '\u23F0' : '\uD83D\uDCF7'}</Icon>
      ...
      <ScanButton onClick={onStartScan}>{isStale ? 'Re-scan now' : 'Scan fridge'}</ScanButton>
      {lastScan && (
        <LastScanInfo>
          Last scan: {lastScan.date} at {lastScan.time} {'\u00B7'} {lastScan.itemsDetected} items
          detected
        </LastScanInfo>
      )}
    </Card>
